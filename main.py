...
def getServerChat(server):  # function to recall the preffered server channel from local file storage
    try:
        ...
        for pos, channel in enumerate(
                server.channels):  # NEEDS CLEANING UP. gets the correct channel object from the channel id
...
async def on_member_remove(member):  # function to run when a member keaves or is removed from a server
    logger.info('member: ' + str(member.id) + ' (' + str(member.name) + ') left server: ' + str(member.server.id))
    ...
    await client.send_message(channel, (
                'Member ' + member.name + ' has left the server... configuration successfully backed up.'))  # outputs confirmation of backup
...
async def on_member_join(member):  # function run upon a new user joining a server
    logger.info('new member: ' + str(member.id) + ' (' + str(member.name) + ') joined server: ' + str(member.server.id))
    ...
    await client.send_message(channel,
                              ('Welcome ' + member.mention + ' to the server!'))  # welcome greeting to the new member
        ...
        for serverRole in member.server.roles:  # loops through every role in the server, to append a role to the user you need the role object (not just the id)
            if str(serverRole.id) == role:  # if the server role is matching then attempt to append the role to the user
                try:
                    ...
                    failed_roles.append(
                        serverRole)  # if the role could not be appended, add to the list of failed roles
...
async def on_message(message):
    ...
    if message.content.startswith('!russianroulette'):
        logger.info('!russianroulette - server: ' + str(message.server.id) + ' | user: ' + str(message.author.id) + ' (' + str(message.author.name) + ')')
        global spam_time
        run_time = time.time()
        ...
        if difference > 30:  # change this value to change the time limit (seconds)
            ...
            if voice_channel is None:
                ...
                invite = await client.create_invite(message.channel,
                                                    max_age=600)  # generates an invite that lasts 10 minutes
                all_members = message.server.members
                valid_members = []
                for member in all_members:
                    if member.bot != True and (
                            member != member.server.owner):  # checks the member is not a bot and is not the server owner
    ...
    if message.content.startswith('!update'):
        logger.info('!update - server: ' + str(message.server.id) + ' | user: ' + str(message.author.id) + ' (' + str(message.author.name) + ')')
    ...
    if message.content.startswith('!testchat'):
        logger.info('!testchat - server: ' + str(message.server.id) + ' | user: ' + str(message.author.id) + ' (' + str(message.author.name) + ')')
    ...
    if message.content.startswith('!unban'):
        ...
        if message.author in server_bans:
            if str(message.author.id) == '158639538468683776-':
                ...
                await client.send_message(current_server.owner,
                                          ("Do you want to unban " + str(message.author.name) + "? (y/n)"))
    ...
    if message.content.startswith('!kick'):
        logger.info('!kick ' + message.content + ' - server: ' + str(message.server.id) + ' | user: ' + str(message.author.id) + ' (' + str(message.author.name) + ')')
...
async def on_server_join(server):  # function to run when the bot joins a new server
    logger.info('Joined server: ' + str(message.server.id))
